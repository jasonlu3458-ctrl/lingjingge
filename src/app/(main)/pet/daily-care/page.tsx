...
export default function PetDailyCarePage() {
    ...
    <div className="min-h-screen bg-zen-beige py-12">
      <div className="max-w-5xl mx-auto px-4">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-[#7a7a7a]">起居有度，健康相随 · 一份给爱宠的日常照护指南</p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-12">
          {CARE_GUIDES.map((guide) => (
            <div
              key={guide.id}
              className="bg-white rounded-xl p-6 border border-[#e8e0d0] hover:shadow-lg transition-all"
            >
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-[#9a9a9a] mt-0.5">{guide.subtitle}</p>
                </div>
              </div>
              <ul className="space-y-2.5">
                {guide.points.map((point, i) => (
                  <li key={i} className="text-sm text-[#5a5a5a] leading-relaxed flex gap-2">
        ...
        <div className="bg-white rounded-xl p-8 border border-[#e8e0d0]">
          <div className="text-center mb-8">
            ...
            <p className="text-sm text-[#7a7a7a]">五道小题，看看你与爱宠的日常默契如何</p>
          </div>

          <div className="space-y-8">
            {QUIZ_QUESTIONS.map((item, qIdx) => (
              <div key={qIdx}>
                <p className="text-base font-medium text-[#2c2c2c] mb-3">
                  <span className="text-amber-600 mr-2">{qIdx + 1}.</span>
                  {item.q}
                </p>
                    ...
                    return (
                      <button
                        ...
                        className={`px-4 py-3 rounded-lg border text-sm text-left transition-all ${
                          selected
                            ? 'border-amber-500 bg-amber-50 text-[#2c2c2c]'
                            : 'border-[#e8e0d0] bg-[#faf7f2] text-[#5a5a5a] hover:border-amber-300'
                        }`}
                      >
                        {opt.text}
                      </button>
          ...
          <div className="mt-8 flex flex-col items-center gap-4">
            <div className="flex gap-3">
              <button
                ...
                className={`px-8 py-3 rounded-full text-sm font-medium transition-all ${
                  allAnswered
                    ? 'bg-[#2c2c2c] text-[#f5f0eb] hover:bg-[#1a1a1a]'
                    : 'bg-[#e8e0d0] text-[#9a9a9a] cursor-not-allowed'
                }`}
                style={{ fontFamily: "'Ma Shan Zheng', cursive, serif" }}
              >
                查看结果
              ...
              <button
                type="button"
                onClick={handleReset}
                className="px-6 py-3 rounded-full text-sm border border-[#d0c8b8] text-[#7a7a7a] hover:bg-[#faf7f2] transition-all"
              >
                重置
              </button>
            </div>
            ...
            {!allAnswered && (
              <p className="text-xs text-[#9a9a9a]">请完成全部 5 道题后查看结果</p>
            )}

            {showResult && (
              <div className="w-full mt-4 p-6 rounded-xl bg-[#faf7f2] border border-amber-200 text-center">
                <p
                  className="text-2xl font-semibold text-[#2c2c2c] mb-2"
                  style={{ fontFamily: "'Ma Shan Zheng', cursive, serif" }}
                >
                  {result.level}
                </p>
                <p className="text-sm text-[#7a7a7a] mb-4">总分：{totalScore} / 10</p>
                <p className="text-sm text-[#5a5a5a] leading-relaxed max-w-xl mx-auto">
                  {result.desc}
                </p>
              </div>
            )}
