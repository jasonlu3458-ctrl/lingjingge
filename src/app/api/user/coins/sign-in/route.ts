...
const SIGN_IN_REWARD = 10;   // 每日签到奖励（灵境丹）
const REWARD_NOTE = '每日签到 +10 灵境丹';
...
export async function POST() {
  ...
  if (!isSupabaseConfigured()) {
    return NextResponse.json({
      ...
      message: `${SIGN_IN_REWARD} 灵境丹已到账（mock）`,
      mock: true,
    });
  }
  ...
  try {
    ...
    if (readErr && (readErr.code === '42P01' || /does not exist/i.test(readErr.message))) {
      return NextResponse.json({
        ...
        coins_awarded: SIGN_IN_REWARD,
        signed_in_today: true,
        message: `${SIGN_IN_REWARD} 灵境丹已到账（表未创建，已 mock）`,
        mock: true,
      });
    }
