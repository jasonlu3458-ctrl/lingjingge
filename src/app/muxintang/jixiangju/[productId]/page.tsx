...
export default function MuxintangProductDetailPage() {
  ...
  const fetchProduct = async () => {
    setLoading(true);
    try {
      const found = MOCK_DATA.find(p => p.id === productId);
  ...
  const handleAddToCart = () => {
    ...
    const cart = JSON.parse(localStorage.getItem('muxintang_cart') || '[]');
    ...
    localStorage.setItem('muxintang_cart', JSON.stringify(cart));
    alert('已加入购物车');
  };
  ...
  const handleBuyNow = () => {
    ...
    localStorage.setItem('muxintang_cart', JSON.stringify(cart));
    router.push('/muxintang/jixiangju/checkout');
  };
    ...
    return (
      <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-[#808080] mb-4">商品不存在</p>
          <Link
            href="/muxintang/jixiangju"
            className="inline-block bg-[#8B4513] text-[#D4AF37] px-6 py-3 rounded-lg hover:bg-[#A0522D] transition-colors"
          >
            返回吉祥馆
          </Link>
        </div>
          ...
          <div className="lg:w-1/2">
            ...
            <div className="flex gap-4">
              ...
              <button
                ...
                disabled={!product.is_active}
                className="flex-1 bg-[#D4AF37] text-[#0a0a0a] py-4 rounded-lg font-medium hover:bg-[#E5C142] transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                ⚡ 立即购买
              </button>
            </div>
