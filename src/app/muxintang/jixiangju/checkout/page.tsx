...
export default function MuxintangCheckoutPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const orders = JSON.parse(localStorage.getItem('muxintang_orders') || '[]');
      orders.unshift(orderData);
      localStorage.setItem('muxintang_orders', JSON.stringify(orders));

      localStorage.removeItem('muxintang_cart');

      router.push('/muxintang/jixiangju/orders');
    } catch (error) {
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div>
            ...
            <button
              ...
              disabled={loading}
              className="w-full mt-6 bg-[#D4AF37] text-[#0a0a0a] py-4 rounded-lg font-medium hover:bg-[#E5C142] transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
            >
              {loading ? (
                <>
