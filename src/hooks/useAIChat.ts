...
export function useAIChat({ type, userRole: serverUserRole, initialMessages = [], acharyaId }: UseAIChatOptions) {
  ...
  const sendMessage = useCallback(async (content: string) => {
    ...
    try {
      ...
      if (acharyaId) {
        headers['x-acharya-id'] = acharyaId;
      }
  ...
  }, [type, isLoading, mounted, trySend, persistMessage, acharyaId]);
